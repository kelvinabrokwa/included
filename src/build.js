...
function traverse(dir) {
  ...
  for (var i in items) {
    ...
    if (type === 'dir') traverse(items[i]);else files.push(items[i]);
  }
}
...
function organize(files) {
  ...
  for (var f in files) {
    ...
    if (incl) indie[files[f]] = incl;
  }

  var clean = {};
  Object.keys(indie).forEach(function (f) {
    clean[fname(f)] = indie[f].map(function (i) {
      return fname(i);
    });
  });
  ...
  var out = { edges: [] };
  out.nodes = Object.keys(table).map(function (f) {
    return { data: { id: f } };
  });
...
function getIncludes(file) {
  var includes = fs.readFileSync(file, { encoding: 'utf8' }).split('\n').filter(function (line) {
    return line.indexOf('#include') === 0 && line.indexOf('<') === -1;
  }).map(function (line) {
